...
public OnATMLoaded()
{
    ...
    {
        cache_get_value_int(i, "id", ATMID[i]);

        cache_get_value_float(i, "posx", ATMPos[i][0]);
        cache_get_value_float(i, "posy", ATMPos[i][1]);
        cache_get_value_float(i, "posz", ATMPos[i][2]);
        cache_get_value_float(i, "angle", ATMPos[i][3]);

        ATMObject[i] = CreateDynamicObject(
            19324,
            ATMPos[i][0],
            ATMPos[i][1],
            ATMPos[i][2],
...
CMD:editatm(playerid, params[])
{
    ...
    if(sscanf(params, "d", id))
        return SendClientMessage(playerid, -1, "Gunakan: /editatm <id>");
